...
namespace MBKC.API.Validators.ShipperPayments
{
    public class GetShipperPaymentsValidator : AbstractValidator<GetShipperPaymentsRequest>
    {
        public GetShipperPaymentsValidator()
        {
            ...
            RuleFor(x => x.SortBy)
                ...
                {
                    PropertyInfo[] properties = typeof(GetShipperPaymentResponse).GetProperties();
            ...
            RuleFor(x => x.Status)
                     ...
                     .Must((x, status) =>
                     {
                         ...
                         return StringUtil.CheckShipperPaymentStatus(status);
            ...
            RuleFor(x => x.PaymentMethod)
                     .Cascade(CascadeMode.StopOnFirstFailure)
                     .Must((x, paymentMethod) =>
                     {
                         if (paymentMethod == null)
                         {
                             return true; // Skip validation when status is null
                         }
                         return StringUtil.CheckPaymentMethod(paymentMethod);
                     }).WithMessage("{PropertyName} is required CASH, CASHLESS");
            #endregion
